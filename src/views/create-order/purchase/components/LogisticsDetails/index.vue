<template>
  <div class="ub ub-ac top-opt w100" v-if="status === 'edit'">
    ...
    <a-button class="ml-1" type="primary" @click="showUploadLogistics = true">
      {{ $t('opts.import') }}
    </a-button>
  ...
  <UploadImport :title="$t('purchase.tab2')" v-model:show="showUploadLogistics" :order-id="orderId" />
</template>
...
const { logisticsColumns } = useList()
...
const showUploadLogistics = ref<boolean>(false)
...
const columnsList = computed(() => {
  if (props.status === 'edit') {
    ...
    return logisticsColumns.filter((item) => item.dataIndex !== 'operation')
  } else {
    return logisticsColumns
  }
})
