<template>
  <div class="modal-wrapper" ref="mymodal">
    ...
    >
      <a-form :model="formData" labelWrap :labelCol="{ style: { width: '120px' } }">
        <a-form-item :label="$t('purchase-pay.columns.mappingOrder')" v-bind="validateInfos.purchaseOrder">
          <div class="select-box" @click="showSelector = true">
            {{ formData.purchaseOrder ? formData.purchaseOrder : $t('selectTips') }}
          </div>
        </a-form-item>
        <a-form-item :label="$t('purchase-pay.columns.payTime')" v-bind="validateInfos.payTime">
          <a-date-picker
            style="width: 100%"
            allowClear
            show-time
        ...
        <a-form-item :label="$t('purchase-pay.columns.payType')" v-bind="validateInfos.payType">
          <a-select
            show-search
            :options="payTypeList"
            allowClear
  ...
  <MapPurchaseOrder v-model:show="showSelector" @success="onSuccess" :selectedIds="selectedIds" from="2" />
</template>
